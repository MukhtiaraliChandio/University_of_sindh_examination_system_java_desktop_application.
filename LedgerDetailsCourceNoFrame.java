 ...
 * @author Salman Ahmed
 */
public class LedgerDetailsCourceNoFrame extends javax.swing.JFrame{

    /**
     * Creates new form LedgerDetailsCourceNoFrame
     */
    
    
    
    DefaultTableModel jmodel;
    public LedgerDetailsCourceNoFrame() {
        initComponents();
        
        getDepartmenrt();
        
        jmodel = (DefaultTableModel)jtable.getModel();
        jmodel.addTableModelListener(new TableModelListener(){
                ...
                public void tableChanged(TableModelEvent tme){
                        ...
                        if(column == 6 | column == 7) return;

                        try{
                                String s = jmodel.getValueAt(rowIndex, 3).toString();

                                int max = Integer.parseInt(jmodel.getValueAt(rowIndex, 5).toString());
                                int obtain = Integer.parseInt(s);

                                if(!(obtain > max) & obtain > 0){

                                        String grade = getGrade(obtain, max);
                                        System.out.println("values "+obtain+" "+max+" "+grade);


                                        jmodel.setValueAt(grade, rowIndex,6);
                                        int qp = getQp(grade);

                                        jmodel.setValueAt(qp, rowIndex, 7);
                                        System.out.println(qp);

                                        LedgerDetailsBean key = (LedgerDetailsBean)jmodel.getValueAt(rowIndex, 0);

                                        LedgerDetailsBean values = new LedgerDetailsBean();
                                        values.setMarksObtain((String)jmodel.getValueAt(rowIndex, 3));
                                        values.setGrade((String)jmodel.getValueAt(rowIndex, 6));
                                        values.setQp((int)jmodel.getValueAt(rowIndex, 7));

                                        table.put(key, values);
                                }
                                else{
                                        MainWorkPlace.Jpane("Invalid Marks Entry!");
                                        jmodel.setValueAt("", rowIndex, 3);
                                }
    ...
    private int getQp(String grade){
    ...
    private String getGrade(int obtain, int total){

            float per = (obtain * 100) / total;
    ...
    private void save(){
                    ...
                    int rows = 0;
                    Enumeration eKey = table.keys();

                    while(eKey.hasMoreElements()){
                            LedgerDetailsBean k = (LedgerDetailsBean)(eKey.nextElement());
                            LedgerDetailsBean v = (LedgerDetailsBean)(table.get(k));

                            try{
                                    rows += DatabaseManager.updateLedgerDetails(k.getSeatListId(), k.getStuRollNo(), k.getSchemeId(), k.getSchemePart(), k.getSemester(), k.getCourseNo(), v.getGrade(), v.getMarksObtain(), v.getQp());
                            }catch(Exception ee){
                                    ee.printStackTrace();
                                    MainWorkPlace.Jpane("Update Error  "+ee.getMessage());
                            }

                    }

                    MainWorkPlace.Jpane(rows+" Marks Updated");
    ...
    private void getBatch(){
            ...
            private void getShiftGroup(){
                    ...
                    TbatchShift.setText(Decode.shiftDecode(beans.getBatchShift()));
                    TbatchGroup.setText(Decode.groupDecode(beans.getBatchGroup()));
            }
            /////***** getShift & Group Load *****/////
    ...
    private void getLedgerDetails(){
            ...
            try{
                    LedgerBean lbeans = DatabaseManager.getLedger(slbeans.getSeatListId());
                    if(lbeans == null)return;

                    Vector v = DatabaseManager.getSchemeDetails(lbeans.getSchemePart(), lbeans.getSchemeId());
                    jList.setListData(v);
    ...
    public void valueChanged(ListSelectionEvent ee){
            ...
            SchemeDetailBean sdbeans = (SchemeDetailBean)jList.getSelectedValue();
            if(sdbeans == null)return;

            try{

                    Vector v = DatabaseManager.getLedgerDetails(sdbeans.getCourseNo(), true);
                    for(int i = 0; i < v.size(); i++){

                            LedgerDetailsBean ldbeans = (LedgerDetailsBean)v.elementAt(i);
                            StudentBean sbeans = DatabaseManager.getStudent(ldbeans.getStuRollNo());

                            Vector v1 = new Vector();

                            // v1.addElement(sbeans.getStuRollno());	//FROM STUDENT
                            v1.addElement(ldbeans);						//FROM LEDGER DETAILS

                            v1.addElement(sbeans.getStuName());			//FROM STUDENT

                            v1.addElement(sbeans.getStuFname());		//FROM STUDENT

                            v1.addElement(ldbeans.getMarksObtain());	//FROM LEDGER DETAILS

                            v1.addElement(ldbeans.getMinMarks());		//FROM LEDGER DETAILS

                            v1.addElement(sdbeans.getMaxMarks());		//FROM SCHEME DETAILS**

                            v1.addElement(ldbeans.getGrade());			//FROM LEDGER DETAILS

                            v1.addElement(ldbeans.getQp());				//FROM LEDGER DETAILS

                            v1.addElement(sdbeans.getCreditHours());	//FROM SCHEME DETAILS**
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Ledger Details By Cource Number");
        ...
        jtable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Roll No", "Name", "Father Name", "Obtain", "Min", "Max", "Grade", "QP", "C.H"
            }
        ) {
                    ...
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addGroup(jPanel3Layout.createSequentialGroup()
                                ...
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(TbatchGroup, javax.swing.GroupLayout.PREFERRED_SIZE, 126, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(jPanel3Layout.createSequentialGroup()
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(LedgerDetailsCourceNoFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
